...
- 可选的目标芯片（esp32、esp32s2、esp32s3、esp32c3、esp32c6、esp32h2 等）。

## 自动探测

- 检查 `idf.py` 是否可用（`IDF_PATH` 已设置且 `export.sh` 已激活）。
- 读取 `sdkconfig` 中的 `CONFIG_IDF_TARGET` 确定当前目标芯片。
...
- 当 `idf.py` 不可用或 `IDF_PATH` 未设置时，返回 `environment-missing`，推荐 `idf-setup`。
...
- 当 ESP-IDF 环境未安装时，推荐用户先使用 `idf-setup`。
