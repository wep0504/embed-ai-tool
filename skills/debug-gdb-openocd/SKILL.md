...
2. 若不确定环境是否就绪，先运行自带脚本 [scripts/gdb_debugger.py](scripts/gdb_debugger.py) 的 `--detect` 模式确认。
3. 根据用户意图选择调试模式：`download-and-halt`（默认）、`attach-only` 或 `crash-context`。
4. 使用 `--elf` 指定符号文件，配合 `--interface` + `--target` 或 `--config` 启动调试。
5. 读取脚本输出的调试结果，重点关注寄存器状态、回溯帧和 Fault 寄存器（crash-context 模式）。
6. 将调试配置和关键观察写回 `Project Profile`，并在需要时交给下游 skill。
