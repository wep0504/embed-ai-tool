...
- `target-response-abnormal`：上传过程中出错、校验失败或设备复位后行为异常。
- 失败时至少提供：烧录命令、端口或探针识别结果、上传日志与校验证据。
...
- Windows 下串口命名为 `COMx`，且常受占用影响，建议烧录前关闭串口监视器。
- Linux 下串口通常为 `/dev/ttyUSBx` 或 `/dev/ttyACMx`，若权限不足需先调整设备权限。
- 对于走 JTAG/SWD 的上传协议，需确保调试器驱动与 `platformio.ini` 中 `upload_protocol` 一致。
